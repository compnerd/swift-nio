...
public enum SocketAddress: CustomStringConvertible {
    ...
    public var port: Int? {
        get {
            switch self {
            case .v4(let addr):
                // looks odd but we need to first convert the endianness as `u_short` and then make the result an `Int`.
                return Int(u_short(bigEndian: addr.address.sin_port))
            case .v6(let addr):
                // looks odd but we need to first convert the endianness as `u_short` and then make the result an `Int`.
                return Int(u_short(bigEndian: addr.address.sin6_port))
            case .unixDomainSocket:
                return nil
            }
        }
        set {
            ...
            case .v4(let addr):
                ...
                mutAddr.sin_port = u_short(newValue ?? 0).bigEndian
                self = .v4(.init(address: mutAddr, host: addr.host))
            case .v6(let addr):
                var mutAddr = addr.address
                mutAddr.sin6_port = u_short(newValue ?? 0).bigEndian
    ...
    public init(unixDomainSocketPath: String) throws {
        ...
        addr.sun_family = ADDRESS_FAMILY(BSDSocket.AF_UNIX)
    ...
    public init(ipAddress: String, port: Int) throws {
        self = try ipAddress.withCString {
            do {
                ...
                try BSDSocket.inet_pton(af: BSDSocket.AF_INET, src: $0, dst: &ipv4Addr)

                var addr = sockaddr_in()
                addr.sin_family = ADDRESS_FAMILY(BSDSocket.AF_INET)
                addr.sin_port = u_short(port).bigEndian
            ...
            do {
                ...
                try BSDSocket.inet_pton(af: BSDSocket.AF_INET6, src: $0, dst: &ipv6Addr)

                var addr = sockaddr_in6()
                addr.sin6_family = ADDRESS_FAMILY(BSDSocket.AF_INET6)
                addr.sin6_port = u_short(port).bigEndian
        ...
        return try host.withCString(encodedAs: UTF16.self) { wszHost in
            return try String(port).withCString(encodedAs: UTF16.self) { wszPort in
                ...
                if let pResult = pResult {
                    ...
                    case AF_INET:
                        return pResult.pointee.ai_addr.withMemoryRebound(to: sockaddr_in.self,
                                                                  capacity: 1) {
