...
#if os(Windows)
            self.messageVector = .allocateAndInitialize(repeating: MMsgHdr(msg_hdr: WSAMSG(), msg_len: 0), count: newValue)
#else
...
#if os(Windows)
            precondition(self.messageVector[i].msg_hdr.namelen != 0,
                         "Unexpected zero length peer name")
#else
...
#if os(Windows)
        let messageVector = UnsafeMutableBufferPointer.allocateAndInitialize(repeating: MMsgHdr(msg_hdr: WSAMSG(), msg_len: 0), count: messageCount)
#else
